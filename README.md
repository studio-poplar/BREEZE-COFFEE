...
- **認証**:
  ...
  - お客さん: 本番ではLIFF (`liff.getAccessToken()`) をサーバー側でLINEのプロフィールAPIに照会して検証します(`liff.getIDToken()`は使っていません — IDトークンはログイン時に一度発行されるだけで自動更新されず、数分後に注文しようとすると期限切れで失敗する不具合が実際に発生したため。アクセストークンはLIFF SDKが裏で自動更新してくれます)。**LINEログインチャネルが未設定の間は、開発用ログイン(名前を入力するだけ)に自動フォールバック**します (`NEXT_PUBLIC_LIFF_ID` が空の間だけ有効)。
...
(`LINE_LOGIN_CHANNEL_ID` はアクセストークン検証への切り替えにともない不要になりました。)
